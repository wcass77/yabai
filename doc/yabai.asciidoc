...
*yabai* is a tiling window manager for macOS supporting binary space partitioning, stack, float, and horizontally scrollable workspaces.
...
*layout* ['bsp|stack|float|scroll']::
    Set the layout of the selected space.
    In *scroll* layout, managed windows are arranged as horizontal columns and the focused column is kept visible while the workspace scrolls.
...
*--swap* '<WINDOW_SEL>'::
    ...
    On *scroll* spaces, this swaps column order.

*--warp* '<WINDOW_SEL>'::
    Re-insert the selected window, splitting the given window. +
    On *scroll* spaces, this reorders the selected column relative to the given column.
...
*--insert* '<DIR_SEL>|stack'::
    ...
    Not supported on *scroll* spaces.
...
*--resize* 'top|left|bottom|right|top_left|top_right|bottom_right|bottom_left|abs:<dx>:<dy>'::
    ...
    On *scroll* spaces, only left/right resizing is supported.
...
*--toggle* 'float|sticky|pip|shadow|split|zoom-parent|zoom-fullscreen|windowed-fullscreen|native-fullscreen|expose|<LABEL>'::
    ...
    The *split* toggle is not supported on *scroll* spaces.

Trackpad Gestures
~~~~~~~~~~~~~~~~~

When the selected space uses the *scroll* layout, yabai can react to private trackpad multitouch callbacks:

- three-finger horizontal swipe scrolls the current workspace by one column
- four-finger horizontal swipe switches to the previous/next user space on the active display

These gestures rely on private macOS APIs and do not suppress Dock's native gesture handling. Disable conflicting macOS trackpad gestures for switching spaces if you want reliable four-finger workspace switching.
