...
*--scroll* 'prev|next|first|last|west|east'::
    Scroll the selected *scroll* space by one column or jump to the first/last column, focusing the target column when the space is visible. +
    Boundary scrolls are no-ops. Use this command from keybindings for scroll workspace navigation.
